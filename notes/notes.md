# Roadmap y Gestión de Tareas del Proyecto
...
- Cuando el orquetsador se ejecuta con cron, no envia notificaciones el entorno de escritorio y la interfaz de usuario esten activas. QUiza las validaciones que se realizan para identificar si hay una interfaz de usuario activa no esta funcionando correctamente, o quiza la ejecución en segundo plano o de la forma enq ue se ejecuta con cron, no permite enviar notificaciones. Hayq ue revisar por que pasa esto. Por ejemplo en los logs enconetre multiples mensajes de que el archivo de configuración estaba vaci, pero en pantalla no vi ninguna de esas notificaciones. Cabe la posibilidad de que no me haya dado cunta, pero honestamente lo dudo, además tampoco veo notificaciones residuales en el panel de notificaciones. ¿Podría ser que lo que este dando problemas es el sistena de notificaciones sea en primer o segundo plano? Lo dudo, parece ser solo un problema de cuando el orquestador se ejecuta con cron, pero vale la pena probarlo.
